...
const config: WebGPUChartConfig<DataPoint> = {
    adapter: {
...
const refresh = () => {
    fetch(
        ...
        .then((json) => {
            type SystemBenchmarks = Map<string, DataPoint[]>; // Benchmark name to data points
            const systems = new Map<string, SystemBenchmarks>();
            for (const commit of json.Commits) {
                for (const benchmark of commit.Benchmarks) {
                    ...
                    const data_points = get_or_create(
                        benchmarks,
                        benchmark_name,
                        () => []
                    );
            ...
            systems_sorted.forEach((benchmarks, system_name) => {
                const chart = get_or_create(charts, system_name, () => {
                    ...
                    return new WebGPUChart<DataPoint>(element, config);
                });
